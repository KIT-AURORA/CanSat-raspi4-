...
# モーター設定
left_motor = Motor(forward=19, backward=13)
right_motor = Motor(forward=18, backward=12)
# GPSの設定
...
def calculate_heading(x, y):
    ...
    logging.info(f"{compass_heading}")
...
def calculate_bearing(current_lat, current_lon, target_lat, target_lon):
    ...
    logging.info(f"{compass_bearing}")
...
def calibrate_magnetometer_with_rotation():
    ...
    地磁気の校正
    """
    logging.info("地磁気センサーのキャリブレーションを開始します。CanSatが自動で5秒旋回します...")
    print(
    ...
    print("キャリブレーション完了")
    logging.info(f"{x_offset},{y_offset},{z_offset}")
...
def main():
    ...
    image_navigartion()
    if gps_coordinates:
        create_map(gps_coordinates, "/home/ozo/background_image2.png")
    left_motor.stop()
    right_motor.stop()
    logging.info("終了")
    print("終了")
